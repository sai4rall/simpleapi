...
class QueryRequest(BaseModel):
    ...
    send_email_flag: Optional[bool] = False  # New flag for email
    recheck_flag: Optional[bool] = False     # New flag for recheck
...
class Api:
    ...
    async def predict_anomalies(self, request: QueryRequest, enable_2min_duration: bool = False, enable_5min_duration: bool = False) -> Dict[str, Any]:
        ...
        try:
            services = request.services if request.services is not None else self.detector.default_services
            namespace = request.namespace if request.namespace is not None else self.detector.default_namespace
            send_email_flag = request.send_email_flag if request.send_email_flag is not None else False
            recheck_flag = request.recheck_flag if request.recheck_flag is not None else False
            return self.detector.predict_anomalies(services, namespace, enable_2min_duration, enable_5min_duration, send_email_flag, recheck_flag)
        except Exception as e:
    ...
    async def get_metric_graph_image(self, metric: str) -> Response:
        ...
        try:
            namespace = self.detector.default_namespace
