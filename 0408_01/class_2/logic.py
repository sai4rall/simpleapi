...
class AnomalyDetector:
    def __init__(self):
        ...
        self.db_path = "anomaly_database.db"
    ...
    def _init_database(self):
        """Initialize SQLite database with an anomalies table."""
    ...
    def save_to_database(self, namespace: str, root_cause: Optional[str], affected_jobs: List[str], timestamp: str):
        """Save root cause and affected jobs to SQLite database."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        affected_jobs_str = ",".join(affected_jobs) if affected_jobs else ""
        cursor.execute(
            "INSERT INTO anomalies (timestamp, namespace, root_cause, affected_jobs) VALUES (?, ?, ?, ?)",
            (timestamp, namespace, root_cause, affected_jobs_str)
        )
        conn.commit()
        conn.close()
        logger.info(f"Saved root cause '{root_cause}' and affected jobs '{affected_jobs_str}' to database for {namespace}")
    ...
    def recheck_anomaly_after_delay(self, services: Dict[str, Dict[str, str]], namespace: str, original_root_cause: Optional[str], models: Dict[str, IsolationForest], G: nx.DiGraph) -> Optional[str]:
        """Wait 5 minutes, re-fetch data, and check if the same root cause persists."""
        logger.info("Waiting 5 minutes to recheck anomaly...")
        time.sleep(300)  # 5 minutes in seconds

        df_predict = self.fetch_prometheus_data(lookback="5m", services=services, namespace=namespace)
        if df_predict.empty:
            logger.warning("No data fetched for recheck")
            return None
        
        df_predict = self.preprocess_data(df_predict, remove_outliers_flag=False)
        if df_predict.empty:
            logger.warning("Preprocessed DataFrame is empty for recheck")
            return None
        
        df_predict = df_predict.fillna(0)
        last_five_min_df = df_predict.tail(5)  # Assuming 1m step, adjust if needed
        ...
        for job in job_names:
            ...
            if job_cols and job in models:
                ...
                if not trained_cols:
                    continue
                preds = models[job].predict(last_five_min_df[trained_cols])
                anomaly_indices = last_five_min_df.index[preds == -1]
                ...
                if len(anomaly_indices) > 0:
                    ...
                    for col in trained_cols:
                        ...
                        if anomaly_mask.any():
                            first_anomaly_time = last_five_min_df.index[anomaly_mask][0]
        ...
        return new_root_cause if new_root_cause == original_root_cause else None
    ...
    def predict_anomalies(self, services: Dict[str, Dict[str, str]], namespace: str, enable_2min_duration: bool = False, enable_5min_duration: bool = False, send_email_flag: bool = False, recheck_flag: bool = False) -> Dict[str, Any]:
        ...
        # New features: Save to database, send email, and recheck
        if root_cause:
            # Save to database
            self.save_to_database(namespace, root_cause, affected_jobs, timestamp)

            # Send email if flag is enabled
            if send_email_flag:
                email_subject = f"Anomaly Detected in {namespace}"
                email_body = f"Root Cause: {root_cause}\nAffected Jobs: {', '.join(affected_jobs)}\nTimestamp: {timestamp}"
                self.send_email(email_subject, email_body)

            # Recheck after 5 minutes if flag is enabled
            if recheck_flag:
                persistent_root_cause = self.recheck_anomaly_after_delay(services, namespace, root_cause, models, G)
                if persistent_root_cause:
                    email_subject = f"Persistent Anomaly in {namespace}"
                    email_body = f"Root Cause '{persistent_root_cause}' persists after 5 minutes.\nAffected Jobs: {', '.join(affected_jobs)}\nTimestamp: {timestamp}"
                    self.send_email(email_subject, email_body)
